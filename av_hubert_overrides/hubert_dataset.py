...
"""
Drop-in replacement for hubert_dataset.py that adds support for a second
augmentation manifest alongside the existing first one.

Two-level random selection policy
----------------------------------
  Level 1  – P(AVH_AUG_PROB):  use any augmentation  (else use base)
  Level 2  – P(0.5):            use aug-manifest-1    (else aug-manifest-2)

Env vars
--------
  AVH_AUG_PROB          float  probability of choosing augmented data (default 0)
  AVH_AUG_MANIFEST      path   first  aug TSV  (e.g. blur B_C_D_E_F)
  AVH_AUG_MANIFEST_2    path   second aug TSV  (e.g. interpolated 200 ms)
  AVH_DEBUG_SAMPLES     int    number of samples to log in detail (default 0)

If only AVH_AUG_MANIFEST is set the behaviour is identical to the original
hubert_dataset.py. AVH_AUG_MANIFEST_2 is fully optional.

Train-cell example
------------------
export AVH_AUG_PROB=0.5
export AVH_AUG_MANIFEST="/content/drive/.../Visemes_B_C_D_E_F_word_roi/data/train_aug.tsv"
export AVH_AUG_MANIFEST_2="/content/drive/.../lrs3_lag/data/train_aug.tsv"
"""
...
def _resolve_path(root: str, p: str) -> str:
    """Match how load_video() joins root+relative, and keep abs paths unchanged."""
    return p if os.path.isabs(p) else os.path.join(root, p)

def _utt_key(audio_id: str) -> str:
    """
    audio_id is items[0] from TSV, e.g.:
      trainval/<vid>/<utt>
      trainval_aligned/<vid>/<utt>
    We match base<->aug by last two path components: <vid>/<utt>.
    """
    audio_id = audio_id.replace("\\", "/")
    parts = audio_id.split("/")
    return "/".join(parts[-2:]) if len(parts) >= 2 else audio_id


def _guess_aug_manifest(manifest_path: str) -> str:
    """
    If manifest_path ends with train.tsv, guess train_aug.tsv next to it.
    Otherwise just try sibling train_aug.tsv.
    """
    d = os.path.dirname(manifest_path)
    base = os.path.basename(manifest_path)
    if base == "train.tsv":
        return os.path.join(d, "train_aug.tsv")
    return os.path.join(d, "train_aug.tsv")


def _read_aug_manifest(aug_manifest_path: str):
    """
    Returns dict: key -> (video_path, audio_path, sz)
    """
    aug = {}
    with open(aug_manifest_path, "r", encoding="utf-8") as f:
        _root = f.readline().strip()
        for line in f:
            line = line.strip()
            if not line:
                continue
            ...
            if k not in aug:
                aug[k] = (video_path, audio_path, sz)
    return aug
...
class AVHubertDataset(FairseqDataset):
    def __init__(
        ...
        self.debug_samples = int(os.environ.get("AVH_DEBUG_SAMPLES", "0"))
        self._debug_seen = 0

        # Counters for tracking aug vs base usage per epoch
        self._aug_used_count = 0
        self._total_count = 0

        # Per-type counters for the two aug manifests
        ...
        # auto-detect train_aug.tsv next to train.tsv if not provided
        if self.aug_prob > 0 and not self.aug_manifest:
            cand = _guess_aug_manifest(manifest_path)
            if os.path.isfile(cand):
                self.aug_manifest = cand

        self.aug_names = [None] * len(self.sizes)  # per-base-index optional (video_fn, audio_fn + ":" + base_audio_id)

        if self.aug_prob > 0 and self.aug_manifest:
            aug_map = _read_aug_manifest(self.aug_manifest)

            n_has_aug = 0
            for i, (v_base, a_mix) in enumerate(self.names):
                # base audio_id lives after ':' in a_mix
                base_audio_id = a_mix.split(":", 1)[1]
                k = _utt_key(base_audio_id)
                if k in aug_map:
                    v_aug, a_aug, sz_aug = aug_map[k]
                    # keep base_audio_id so utt_id/labels remain consistent
                    self.aug_names[i] = (v_aug, a_aug + ":" + base_audio_id)

                    # batching safety (pad_audio=True uses self.sizes for batch-by-tokens)
                    if sz_aug > self.sizes[i]:
                        self.sizes[i] = sz_aug

                    n_has_aug += 1

            logger.info(
                f"[AUG-MIX] aug_prob={self.aug_prob} aug_manifest={self.aug_manifest} "
                f"aug_available_for={n_has_aug}/{len(self.sizes)} base samples"
            )
        else:
            logger.info(f"[AUG-MIX] disabled (AVH_AUG_PROB={self.aug_prob}, AVH_AUG_MANIFEST={self.aug_manifest or 'auto'})")

        # ------------------------------------------------------------------
        # Second augmentation manifest (optional)
        # ------------------------------------------------------------------
        self.aug_manifest_2 = os.environ.get("AVH_AUG_MANIFEST_2", "").strip()
        self.aug_names_2 = [None] * len(self.sizes)

        if self.aug_prob > 0 and self.aug_manifest_2:
            if not os.path.isfile(self.aug_manifest_2):
                logger.warning(
                    f"[AUG-MIX-2] AVH_AUG_MANIFEST_2={self.aug_manifest_2} "
                    f"not found – second aug disabled"
                )
            else:
                aug_map_2 = _read_aug_manifest(self.aug_manifest_2)
                n_has_aug2 = 0
                for i, (_, a_mix) in enumerate(self.names):
                    base_audio_id = a_mix.split(":", 1)[1]
                    k = _utt_key(base_audio_id)
                    if k in aug_map_2:
                        v_aug2, a_aug2, sz_aug2 = aug_map_2[k]
                        self.aug_names_2[i] = (v_aug2, a_aug2 + ":" + base_audio_id)
                        if sz_aug2 > self.sizes[i]:
                            self.sizes[i] = sz_aug2
                        n_has_aug2 += 1
                logger.info(
                    f"[AUG-MIX-2] aug_manifest_2={self.aug_manifest_2} "
                    f"aug_available_for={n_has_aug2}/{len(self.sizes)} base samples"
                )
        else:
            logger.info(
                f"[AUG-MIX-2] disabled "
                f"(AVH_AUG_MANIFEST_2={self.aug_manifest_2 or 'not set'})"
            )
    ...
    def load_feature(self, mix_name):
        ...
        if 'audio' in self.modalities:
            ...
            audio_fn = _resolve_path(self.audio_root, audio_fn)
    ...
    def __getitem__(self, index):
        mix = self.names[index]

        used_aug = False
        aug_source = 0  # 0=base, 1=aug1, 2=aug2

        # Train-only behavior: in fairseq, train dataset uses shuffle=True.
        # Keep valid/test deterministic (typically shuffle=False).
        if self.shuffle and self.aug_prob > 0:
            # Level 1: should we use augmentation at all?
            if np.random.rand() < self.aug_prob:
                aug1 = self.aug_names[index]
                aug2 = self.aug_names_2[index]

                # Level 2: pick which aug (50/50 when both exist)
                if aug1 is not None and aug2 is not None:
                    if np.random.rand() < 0.5:
                        chosen, chosen_source = aug1, 1
                    else:
                        chosen, chosen_source = aug2, 2
                elif aug1 is not None:
                    chosen, chosen_source = aug1, 1
                elif aug2 is not None:
                    chosen, chosen_source = aug2, 2
                else:
                    chosen = None
                    chosen_source = 0
                ...
                if chosen is not None:
                    v_aug, a_aug_mix = chosen
                    a_aug = a_aug_mix.split(":", 1)[0]
                    v_aug_path = _resolve_path(self.audio_root, v_aug)
                    a_aug_path = _resolve_path(self.audio_root, a_aug)

                    if os.path.exists(v_aug_path) and os.path.exists(a_aug_path):
                        mix = chosen
                        used_aug = True
                        aug_source = chosen_source
                    else:
                        # Disable the missing entry to avoid repeated stat calls
                        if chosen_source == 1:
                            self.aug_names[index] = None
                        else:
                            self.aug_names_2[index] = None

        video_feats, audio_feats = self.load_feature(mix)
        ...
        if self.debug_samples > 0 and self._debug_seen < self.debug_samples:
            self._debug_seen += 1

            video_fn, audio_mix = mix
            audio_fn, fid_dbg = audio_mix.split(":", 1)  # audio path, then fid

            v_path = _resolve_path(self.audio_root, video_fn)
            a_path = _resolve_path(self.audio_root, audio_fn)

            raw_label = self.label_list[0][index] if self.store_labels else "(streamed)"
            proc_label = labels[0]
            source_tag = {0: "BASE", 1: "AUG1", 2: "AUG2"}[aug_source]

            logger.info(
                f"[DEBUG] sample idx={index} used={source_tag} "
                f"fid={fid_dbg} video={v_path} audio={a_path} "
                f"video_shape={None if video_feats is None else tuple(video_feats.shape)} "
                f"audio_shape={None if audio_feats is None else tuple(audio_feats.shape)} "
                f"raw_label={raw_label} proc_label={proc_label}"
            )

        # keep fid stable (base audio_id)
        fid = mix[1].split(':')[1]
        return {
            "id": index,
            'fid': fid,
            ...
            "label_list": labels,
            "used_aug": used_aug,
            "aug_source": aug_source,  # 0/1/2 – used by collater for per-type tracking
        }
    ...
    def collater(self, samples):
        ...
        # Track aug vs base usage in main process
        for s in samples:
            self._total_count += 1
            if s.get("used_aug", False):
                self._aug_used_count += 1
            src = s.get("aug_source", 0)
            if src == 1:
                self._aug1_used_count += 1
            elif src == 2:
                self._aug2_used_count += 1

        logger.info(f"[collater] batch_size={len(samples)}, total_count={self._total_count}, aug_count={self._aug_used_count}")
    ...
    def ordered_indices(self):
        # Log aug vs base usage from previous epoch if we have data
        logger.info(f"[ordered_indices] called: shuffle={self.shuffle}, _total_count={self._total_count}, _aug_used_count={self._aug_used_count}")
        if self._total_count > 0:
            base_count = self._total_count - self._aug_used_count
            aug_pct = (self._aug_used_count / self._total_count) * 100
            logger.info(
                f"[EPOCH-SUMMARY] Base: {base_count}/{self._total_count} | "
                f"Aug: {self._aug_used_count}/{self._total_count} ({aug_pct:.1f}%)"
            )
            # Per-type breakdown (only meaningful when AVH_AUG_MANIFEST_2 is set)
            if self.aug_manifest_2:
                logger.info(
                    f"[EPOCH-SUMMARY-MULTI] "
                    f"Aug1: {self._aug1_used_count}/{self._total_count} | "
                    f"Aug2: {self._aug2_used_count}/{self._total_count} | "
                    f"Base: {base_count}/{self._total_count}"
                )
